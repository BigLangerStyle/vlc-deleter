...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'src/vlc-archive.lua') -Destination $extensions -Force
foreach ($name in @('Delete-File.ps1', 'Favorite-File.ps1', 'Archive-File.ps1')) {
...
Write-Output "VLC Deleter is installed. Restart VLC to find Delete current video, Favorite current video, and Archive current video in the View menu."
